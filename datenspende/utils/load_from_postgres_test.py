from datenspende.utils import create_pg_db_connection, teardown_pg_db_connection, query_pg_df
import pandas as pd


def test_create_db_connection_is_open_and_can_be_closed():
    connection = create_pg_db_connection()
    assert connection.closed == 0

    teardown_pg_db_connection(connection)
...
def test_can_execute_sql_on_connection():
    ...
    df = pd.read_sql(
        "select * from datenspende_derivatives.homogenized_features LIMIT 10;", conn
    )
